...
with open("dataset_2018_mu.json","r") as js:
	...
	for key in jj.keys():
		...
		for skey in subkey:
			if len(flist) >= 2:
				...
				with open("dataset_2018_mu/%s%s.dat"%(key,skey),"wb") as output:
...
signal_filelist = pickle.load(open("dataset_2018_mu/run2018A_mu1aa.dat","rb"))

...
with open("dataset_2018_eg.json","r") as js:
	...
	for key in jj.keys():
		...
		for skey in subkey:
			if len(flist) >= 2:
				...
				with open("dataset_2018_eg/%s%s.dat"%(key,skey),"wb") as output:
...
signal_filelist = pickle.load(open("dataset_2018_eg/run2018A_eg1aa.dat","rb"))

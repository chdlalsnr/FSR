...
with open("dataset_2018_mm.json","r") as js:
	...
	for key in jj.keys():
		...
		for skey in subkey:
			if len(flist) >= 2:
				...
				with open("dataset_2018_mm/%s%s.dat"%(key,skey),"wb") as output:
...
signal_filelist = pickle.load(open("dataset_2018_mm/run2018A_mm1aa.dat","rb"))

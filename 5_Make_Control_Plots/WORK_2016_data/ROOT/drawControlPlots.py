...
#	ff.Close()
	...
	c1 = TCanvas("c1","Stacked Histograms of Basic Kinematics l2", 900,600)

	c1.Divide(2,2)
        c1.cd(1)
	stack_l2_mass.Draw("hist")
	sel = hreal_l2_mass.GetMaximumBin()
        ylim = hreal_l2_mass.GetBinCenter(sel)
        stack_l2_mass.GetYaxis().SetRangeUser(0,ylim+1000)
	stack_l2_mass.GetXaxis().SetTitle("Invariant Mass[GeV]")
	stack_l2_mass.GetYaxis().SetTitle("Events/20GeV")
	hsig_l2_mass.SetLineColor(kRed)
	hsig_l2_mass.Draw("hist same")
	hreal_l2_mass.SetLineColor(kBlack)
        hreal_l2_mass.Draw("hist e1 same")
#	err_l2_mass.SetFillColor(kBlue)
#	err_l2_mass.SetFillStyle(3004)
#	err_l2_mass.Draw("E2 same")
	lst_l2_mass.Draw()

	c1.cd(2)
	stack_l2_pt.Draw("hist")
	sel = hreal_l2_pt.GetMaximumBin()
        ylim = hreal_l2_pt.GetBinCenter(sel)
        stack_l2_pt.GetYaxis().SetRangeUser(0,ylim+1000)
        stack_l2_pt.GetXaxis().SetTitle("Transverse Momentum[GeV]")
        stack_l2_pt.GetYaxis().SetTitle("Events/20GeV")
        hsig_l2_pt.SetLineColor(kRed)
        hsig_l2_pt.Draw("hist same")
	hreal_l2_pt.SetLineColor(kBlack)
        hreal_l2_pt.Draw("hist e1 same")
#        err_l2_pt.SetFillColor(kBlue)
#        err_l2_pt.SetFillStyle(3004)
#        err_l2_pt.Draw("E2 same")
        lst_l2_pt.Draw()

	c1.cd(3)
	stack_l2_eta.Draw("hist")
	sel = hreal_l2_eta.GetMaximumBin()
        ylim = hreal_l2_eta.GetBinCenter(sel)
        stack_l2_eta.GetYaxis().SetRangeUser(0,ylim+1000)
        stack_l2_eta.GetXaxis().SetTitle("Eta")
        stack_l2_eta.GetYaxis().SetTitle("Events/.24")
        hsig_l2_eta.SetLineColor(kRed)
        hsig_l2_eta.Draw("hist same")
	hreal_l2_eta.SetLineColor(kBlack)
        hreal_l2_eta.Draw("hist e1 same")
#        err_l2_eta.SetFillColor(kBlue)
#        err_l2_eta.SetFillStyle(3004)
#        err_l2_eta.Draw("E2 same")
        lst_l2_eta.Draw()

	c1.cd(4)
	stack_l2_phi.Draw("hist")
	sel = hreal_l2_phi.GetMaximumBin()
        ylim = hreal_l2_phi.GetBinCenter(sel)
        stack_l2_phi.GetYaxis().SetRangeUser(0,ylim+1000)
        stack_l2_phi.GetXaxis().SetTitle("Phi")
        stack_l2_phi.GetYaxis().SetTitle("Events/.32rad")
        hsig_l2_phi.SetLineColor(kRed)
        hsig_l2_phi.Draw("hist same")
	hreal_l2_phi.SetLineColor(kBlack)
        hreal_l2_phi.Draw("hist e1 same")
#        err_l2_phi.SetFillColor(kBlue)
#        err_l2_phi.SetFillStyle(3004)
#        err_l2_phi.Draw("E2 same")
        lst_l2_phi.Draw()

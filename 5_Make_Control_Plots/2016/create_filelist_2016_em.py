...
with open("dataset_2016_em.json","r") as js:
	...
	for key in jj.keys():
		...
		for skey in subkey:
			if len(flist) >= 2:
				...
				with open("dataset_2016_em/%s%s.dat"%(key,skey),"wb") as output:
...
signal_filelist = pickle.load(open("dataset_2016_em/run2016B_em1aa.dat","rb"))

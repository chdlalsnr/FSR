...
with open("dataset_2016_ee.json","r") as js:
	...
	for key in jj.keys():
		...
		for skey in subkey:
			if len(flist) >= 2:
				...
				with open("dataset_2016_ee/%s%s.dat"%(key,skey),"wb") as output:
...
signal_filelist = pickle.load(open("dataset_2016_ee/run2016B_ee1aa.dat","rb"))

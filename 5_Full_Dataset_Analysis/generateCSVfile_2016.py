...
#				temp_fatpt.append(t.FatJet_pt.__getitem__(j))
			...
			for k in range(t.nMuon):
                                ...
                                if (temp_pt[k] > 25 and abs(temp_eta[k]) <= 2.4 and abs(temp_dxy[k]) <= 0.05 and abs(temp_dz[k]) <= 0.1): # temp_medid[k] == True
                                        for l in range(k+1,t.nMuon):
                                                if (temp_pt[l] > 15 and abs(temp_eta[l]) < 2.4 and abs(temp_dxy[k]) <= 0.05 and abs(temp_dz[k]) <= 0.1):
...
#			higgs_select = min(mass_cand)
			...
			for k in range(t.nJet):
                                ...
                                if (temp_pt[k] >= 20 and abs(temp_eta[k]) < 2.4 and temp_btag[k] > 0.3093): # temp_jetid[k] == 7
                                        for l in range(k+1,t.nJet):
						if (temp_pt[l] >= 20 and abs(temp_eta[l]) < 2.4 and temp_btag[l] > 0.3093):

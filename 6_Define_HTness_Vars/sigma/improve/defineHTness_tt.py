	...
	for fname in flist:
		...
		for i in range(event_num):
			...
			ini = np.array([0,0,0, 0,0,0])
                        ...
                        opt = minimize(defineTopness, ini, constraints=cons, bounds=bnd, method='SLSQP', options={'ftol':20}) #'disp':True})
			...
			def findTopCombination():
                                ...
                                if (abs(opt2-opt.fun) >= abs(opt1-opt.fun)): flag_top = True
                                if (abs(opt1-opt.fun) > abs(opt2-opt.fun)) : flag_top = False
